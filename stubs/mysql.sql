...
## profile mapping
DROP TABLE IF EXISTS install_profile_map;
CREATE TABLE install_profile_map (
	installId BIGINT PRIMARY KEY,
	profileId       INT,
	FOREIGN KEY (profileId) REFERENCES profile (profileId)
		ON DELETE CASCADE # if profile is deleted, revert to fallback, i.e. org default
);
DROP TABLE IF EXISTS repo_profile_map;
CREATE TABLE repo_profile_map (
	repoId BIGINT UNSIGNED PRIMARY KEY,
	profileId    INT,
	FOREIGN KEY (profileId) REFERENCES profile (profileId)
		ON DELETE CASCADE
);
